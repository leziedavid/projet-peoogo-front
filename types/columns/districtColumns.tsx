...
import { DistrictResponse } from "../ApiReponse/ListeResponse";

export const columns: ColumnDef<DistrictResponse>[] = [
    {
        accessorKey: "nom",
        header: ({ column }) => (
            <Button variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === "asc") }> Non du district
                <ArrowUpDown className="ml-2 h-4 w-4" />
            </Button>
        ),
        cell: ({ row }) => (
            <span className="font-semibold">{row.original.nom}</span>
        ),
    },
