...
import parsePhoneNumberFromString, { AsYouType, type CarrierCode, type CountryCallingCode, type E164Number, type NationalNumber, type CountryCode, type NumberType, } from "libphonenumber-js";
									...
									{countries.map((country) => {
										return (
											<CommandItem key={country.iso3} value={`${country.name} (+${country.phone_code})`}
												onSelect={() => {
													if (inputRef.current) { inputRef.current.value = `+${country.phone_code}`; handlers.set(`+${country.phone_code}`); inputRef.current.focus(); }
												...
												<Image src={`/flags/${country.iso2.toLowerCase()}.svg`} width={180} height={180} className="relative top-0.5 mr-2 w-4 h-3 object-cover" aria-labelledby={country.name} title={country.name} alt={country.name} priority />
												{country.name}
