...
import { Upload as IconUpload, X, Camera, Image as ImageIcon } from "lucide-react";
...
export function FileUploader({
    ...
    const handleFiles = (fileList: FileList | null) => {
        ...
        let updated: File[];
        if (multiple) {
            // Plusieurs fichiers → on empile
            updated = [...files, ...newFiles];
        } else {
            // Un seul fichier → on remplace
            updated = newFiles.slice(0, 1);
        }

        handleChange(updated); // ✅ utilise ton callback déjà prévu
            ...
            {mobile ? (
                ...
                <Dropzone onDrop={onDrop} accept={accept} maxSize={maxSize} maxFiles={maxFiles} multiple={multiple} disabled={isDisabled} >
                    ...
                    {({ getRootProps, getInputProps, isDragActive }) => (
                        <div
                            {...getRootProps()} className={cn("group border-muted-foreground/25 hover:bg-muted/25 relative grid w-full cursor-pointer place-items-center rounded-lg border-2 border-dashed px-5 py-2.5 text-center transition", "ring-offset-background focus-visible:ring-ring focus-visible:ring-2 focus-visible:ring-offset-2",
